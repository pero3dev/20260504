...
- **A5 follow-up³⁰ cognito output に `federation_audience_csv` を追加(²⁹ の terraform 直接接続経路を仕上げ)**(²⁹ で IB 側を CSV 許可リストに対応させたが、 cognito 側 output は `app_client_ids` map のままで、 operator が自分で `join(",", values(...))` する必要があった。 ここを 1 step で `terraform output -raw federation_audience_csv | kubectl create secret` まで通すよう仕上げ):
    ...
    - **`infra/cognito/terraform/README.md`** 出力対応表: `federation_audience_csv` 行を追加
    - **terraform fmt + validate 緑** (local terraform 1.7.5)、 既存 `terraform-ci.yml` の `validate-cognito` job が PR 時に再走する経路に乗る
- **A5 follow-up²⁹ Identity Broker `FEDERATION_AUDIENCE` を CSV 許可リスト化(multi-client 本番投入のセキュリティギャップを閉じる)**(`infra/cognito/terraform/outputs.tf` の `app_client_ids` は 4 web app それぞれの client_id を map で吐く一方、 `NimbusIdpTokenVerifier` は単一 String しか受けず、 README にも「共通親に集約する場合は別」 と未解決のまま残っていた。 つまり multi-client 構成で本番投入する瞬間に audience 検証を OFF にせざるを得ない状態 = セキュリティ後退。 ²⁹ で多値 allow-list 化して閉じる):
    - **`NimbusIdpTokenVerifier` 改修**: `expectedAudienceValue` (String) → `expectedAudienceValues` (`List<String>`)。 constructor の `@Value("${platform.identity.federation.audience:}")` は引き続き String CSV を受けるが、 内部で `parseCsv` (split "," + trim + filter empty) で immutable List 化。 verify() の audience 比較は `list.contains(aud)` の allow-list 方式に変更。 単一値 ("abc123") も後方互換で 1 要素 list として通る
    - **post-decode ロジックを package-private `verifyDecoded(Jwt)` に抽出**: 単体テストで Nimbus 実 decode 経路を介さず audience / subject / groups の取り出しを直接 assert するため。 `verify(String)` は引き続き Nimbus decode → `verifyDecoded(Jwt)` に delegate
    - **新規 `NimbusIdpTokenVerifierTest` 9 ケース** (既存テスト無しだったので新設): audience 未設定 skip、 単一一致、 単一不一致 reject、 CSV 複数いずれか一致、 CSV 複数全外し reject、 CSV 前後空白 + 空要素除去、 audience 設定済 + token 欠落 reject、 subject 欠落 reject、 groups list 取り出し
    - **`infra/k8s/identity-broker/federation-configmap.yaml` 更新**: 「単一親 client が必要」 という旧コメントを削除し、 「`app_client_ids` を CSV で列挙」 + 例値の docstring に書き換え
    - **`infra/cognito/README.md` / `infra/cognito/terraform/README.md` 更新**: `FEDERATION_AUDIENCE=<step 4 の CLIENT_ID>` の単一値前提を `<CLIENT_ID_1>,<CLIENT_ID_2>,...` に書き換え、 cognito output 表の「`app_client_ids[*]`」 行を「全値 CSV、 multi-client 対応」 に
    - **`services/identity-broker/src/main/resources/application.yml` 更新**: `audience:` 行に CSV 形式 + 空時 skip の inline コメント追加
    - **trade-off**: 「audience を List 化」 で `@ConfigurationProperties` への移行も検討したが、 既存 `@Value` の他 6 項目を巻き添えにする refactor 規模が大きいため見送り。 String CSV のまま受けて内部で split する小幅変更で multi-client gap を閉じ、 将来 `@ConfigurationProperties` 移行は別 phase に
...
- **A5 follow-up²⁷ DailyMerkleAnchorScheduler にも Micrometer metrics を追加(²⁶ の対称形、 write/read 双方の sweep を可視化)**(²⁶ で verify (read) 側に metrics を入れた対称として、 compute (write) 側にも metrics を入れる。 compute scheduler が止まると verify は ANCHOR_NOT_FOUND を出し続けるため、 compute 側の不在を先に検知できないと verify メトリクスから間接推測になる。 J-SOX 継続監視層を write/read 双方向で機械可読に):
    ...
    - **新規テスト 4 ケース** (`DailyMerkleAnchorSchedulerTest`、 既存テスト無しなのでファイル新設): tenants 空の no-op、 各 tenant 1 回呼出、 `created` / `already_anchored` の status 別 counter 検証、 RuntimeException catch + 他 tenant 続行 + exception counter 検証。 `MeterRegistry` を `SimpleMeterRegistry` で注入
    - **`MeterRegistry` を constructor 必須引数化**: ²⁶ と同じ pattern。 actuator + micrometer-core が `commons-observability` 経由で classpath 既存。 production では auto-config の `MeterRegistry` Bean が解決
    - **trade-off**: tag cardinality は `tenant × 2 status` で線形 (例: 100 tenants → 200 series)。 verify 側の `tenant × 4 status` と合わせても Datadog standard cardinality 制限内。 status タグは verify 側 (`ok` / `*_mismatch` / `anchor_not_found`) と compute 側 (`created` / `already_anchored`) で値域が直交し、 metric 名で分けているのでぶつからない
- **A5 follow-up²⁶ MerkleAnchorVerifyScheduler に Micrometer metrics を追加(Datadog 可視化 / アラート化)**(²⁵ で入れた verify scheduler の結果は log line のみで、 Datadog dashboard / アラート化するには log grep に依存していた。 J-SOX 継続監視層を「機械可読な metric で監視」 する形に格上げし、 mismatch != 0 を SLO 化できるようにする):
    - **counter `audit.anchor.verify.count`** — 1 verify 完走毎に +1。 タグ `tenant` (multi-tenant 集計対応) + `status` (`ok` / `anchor_not_found` / `record_count_mismatch` / `root_mismatch`)。 mismatch 系を Datadog で「直近 25 時間の合計 != 0」 でアラート化する想定(scheduler 周期 + 1h 余裕)
    - **counter `audit.anchor.verify.exceptions`** — verify 中の `RuntimeException` を catch するたびに +1、 タグ `tenant` のみ。 DB 切断などの運用障害を J-SOX mismatch と区別して可視化する(運用障害が発生しても J-SOX SLO 違反の誤検知にならない)
    - **`MeterRegistry` を constructor 必須引数化**: 既存 5 ケース全てに `SimpleMeterRegistry` を注入する形に書き換え。 audit-service は `commons-observability` 経由で actuator + micrometer-core が classpath にいるので production では auto-config の `MeterRegistry` Bean が解決される。 Datadog exporter が wire されれば `audit.anchor.verify.*` がそのまま Datadog に流れる
    - **テスト 1 ケース追加 (合計 6)**: `metrics_は_status別_に_tenant_tag_付きで_counter_を_発行する` で 4 status + 1 exception を 1 sweep に詰めて `SimpleMeterRegistry` から tag 付き counter 値を assert
    - **trade-off**: tag cardinality は `tenant × 4 status` で線形増加 (例: 100 tenants → 400 series)。 Datadog の standard cardinality 制限内に余裕、 status タグを別 metric 名に展開する微 split は不要と判断。 exception counter は status タグを持たないため別 metric に分離(高 cardinality な exception class タグを意図的に持たせない、 J-SOX SLO 計算式を単純化)
...
- **A5 follow-up²² admin が user の現状 revocation 状態を確認する read API**(¹⁹ 完了時点で「revocation 状態の admin 確認 API は MVP では `redis-cli GET` で代用」 と TODO 残しにしていた最後のピース。 これで ADR-0023 関連の admin REST 表面が write (revoke) + read (status) の対称になる):
    ...
    - **port-in**: `GetUserRevocationStatusUseCase` を新設。 内部 record `RevocationStatus(boolean revoked, Long ttlSeconds)`。 `getStatus(long)` メソッド(`GetUserUseCase.get(long)` と signature 衝突回避のため命名差別化)
    - **`UserManagementService` に 7 つ目の port を実装**。 `@Transactional(readOnly=true)` + `@Auditable("USER_REVOCATION_STATUS_GET", read=true)` で admin の参照行為自体を audit chain に記録。 user 存在確認 → `revocationStore.getRevocationTtl(id)` → `Optional<Duration> → RevocationStatus` 変換
    - **`UserAdminController.getUserRevocationStatus`** 実装、 7 つ目の use case を constructor 注入。 OpenAPI generator が `RevocationStatusResource` DTO を出力
    - **テスト追加**: `UserManagementServiceTest` 3 ケース(revoke 中 / revoke 無 / 該当無 user)、 `RevocationE2EIntegrationTest` 2 ケース(IT で実 Redis に key を立てて status read → `revoked=true` + `ttlSeconds` 数値、 立てずに read → `revoked=false` + `ttlSeconds` 不在)。 `AdminSecurityTest` に `@MockitoBean GetUserRevocationStatusUseCase` 追加。 全 70 + 6 通過
    - **これで ADR-0023 関連 admin 操作面が完結**: lifecycle-driven 自動 revoke 3 系統(`user.deactivate` / `removeMembership` / `tenant deactivate fanout`)+ admin 強制 revoke (¹⁹) + admin status read (²²) = write 4 経路 + read 1 経路。 `redis-cli GET revocation:user:<id>` 直叩き運用は不要に
- **A5 follow-up²¹ GitHub Actions を Node.js 24 対応版に bump**(CI annotation で「Node.js 20 actions are deprecated. ... Actions will be forced to run with Node.js 24 by default starting June 2nd, 2026」 と警告が出続けていた件を、 deadline (2026-06-02) 約 3 週間前に解消):
    - **bump 対象 actions** (5 ファイル合計 16 箇所): `actions/checkout@v4 → v5` / `actions/setup-java@v4 → v5` / `actions/upload-artifact@v4 → v5` / `actions/setup-node@v4 → v5`。 v5 系は `actions/checkout@v5` の release note `Update actions checkout to use node 24` の通り Node 24 バンプのみで API 互換、 v6/v7 を選ばず最小 bump に留めた
    - **bump 不要だった actions**: `github/codeql-action/init@v3` / `github/codeql-action/analyze@v3` / `hashicorp/setup-terraform@v3` / `pnpm/action-setup@v4` は action.yml の `using:` を確認したところ既に `node24` で動作中(deprecation warning 対象外)
    - **副作用対応**: `setup-node@v5` は `package.json` の `packageManager` フィールド (我々は `pnpm@9.15.0`) 検出で auto-cache を有効化する仕様に変わったが、 frontend は MVP 段階で `pnpm-lock.yaml` を commit していない (`--no-frozen-lockfile` 運用) ため cache key 算出で失敗する。 `package-manager-cache: false` で明示無効化、 lockfile commit 後に有効化する旨を comment に残す
    - **検証**: 全 5 workflow (ci / codeql / frontend / pact-broker / terraform) で grep 結果 `actions/*@v4` が 0 件、 `*@v5` のみ。 main push CI で deprecation warning が消えることを確認
- **A5 follow-up²⁰-hotfix Redis 有り service で `NoOpRevocationStore` が誤って優先される問題を修正**(²⁰ の IT が CI で 4 中 3 失敗。 root cause: `PlatformSecurityAutoConfiguration` 内で `noOpRevocationStore` が外側 method、 `redisRevocationStore` は nested `@Configuration` 内 method として宣言されており、 Spring Boot が外側 → 内側の順で `@ConditionalOnMissingBean(RevocationStore.class)` を評価するため、 Redis classpath + Bean 両方が揃った identity-broker でも先に NoOp が登録され、 `RevocationCheckFilter` が常に「未 revoke」 として通していた。 ¹⁷-hotfix で隔離した nested config が ²⁰ で逆に優先順位の問題を生む形だった):
    - **fix**: `RevocationStore` の Bean 提供を 3 つの別ファイルの `@AutoConfiguration` に分離し、 `@AutoConfiguration(after=...)` で評価順序を明示
        - `RedisRevocationStoreAutoConfiguration` — class-level `@ConditionalOnClass({StringRedisTemplate, RedisConnectionFactory})` で Redis 不在 service では class-load 自体を起こさせない。 `afterName=RedisAutoConfiguration` で Spring Boot の `RedisConnectionFactory` Bean が登録された後に評価される(でないと `@ConditionalOnBean(RedisConnectionFactory)` が false 返却)
        - `NoOpRevocationStoreAutoConfiguration` — `after=RedisRevocationStoreAutoConfiguration.class` で Redis 評価後に動作。 Redis 変種が登録済みなら `@ConditionalOnMissingBean(RevocationStore.class)` で skip
        - `PlatformSecurityAutoConfiguration` — `after={Redis…, NoOp…}.class` で両者の後に評価。 `RevocationCheckFilter` Bean のみ提供
    - **`META-INF/.../AutoConfiguration.imports`** に 3 行登録(従来 1 行から拡張)
    - **新 regression test**: `Redis_classpath_有り_かつ_RedisConnectionFactory_Bean_有り_なら_RedisRevocationStore_を採る`(`RedisAutoConfiguration` を `ApplicationContextRunner` に同梱して `LettuceConnectionFactory` を生成 → Redis 変種が選ばれることを assert)。 既存 2 ケース(Redis 不在 → NoOp / Redis classpath 有 + Bean 無 → NoOp)もそのまま green
    - **`AdminSecurityTest`** が `@WebMvcTest` で `@Import(PlatformSecurityAutoConfiguration.class)` していたため、 NoOp Bean を別 import に追加。 commons-security の Bean 提供を 3 file に分けた以上、 該当 import 側も追従する必要がある(他 service の SecurityConfig は spring.factories 経由なので影響なし)
    - **²⁰ IT 4 ケースが CI で初実行通過する見込み**(local Windows は Docker 不在で auto-skip 維持)
- **A5 follow-up²⁰ identity-broker 単一 context IT で ADR-0023 通し path を初実機検証**(¹⁷-¹⁹ までは Mockito で `revocationStore.revokeUser` の引数を verify するだけで、 `RevocationCheckFilter` (commons-security) → `RedisRevocationStore` → Spring Security 401 RFC 7807 という通し path は 1 度も実機で動かしていなかった。 単一 Spring context + Postgres + Redis Testcontainers で初の通し検証を入れる):
    - **`services/identity-broker/src/test/java/.../e2e/RevocationE2EIntegrationTest`** を新設(4 ケース)。 `inventory-core/KafkaIntegrationE2ETest` と同型の単一 context IT パターンに揃える(canonical CI Kafka validation と同じく `*Test.java` 命名で surefire 実行 + `@Testcontainers(disabledWithoutDocker=true)` で Windows 自動 skip / CI Linux で実機実行)
    - **container 構成**: `PostgreSQLContainer<>("postgres:16-alpine")` + `GenericContainer<>("redis:7-alpine").withExposedPorts(6379)`。 commons-test の transitive で `org.testcontainers:testcontainers` (GenericContainer) は既に classpath に乗っているため 追加依存無し
    - **`@DynamicPropertySource`** で `spring.datasource.*` + `spring.data.redis.host` / `port` を runtime port に注入。 `@BeforeAll` で Flyway を test 側から手動起動(本番 yml の `spring.flyway.enabled=false` を継承するため)
    - **fixture 設計**: 3 user (admin / victim / innocent) を `@BeforeEach` で seed、 各テスト前に Redis の前回 revocation キーを明示削除。 `revocationStore`(Mockito)を撤廃して real Redis に書く / 読む通し検証
    - **テストケース**: (1) `revoke_tokens_API_は_Redis_に_revocation_キーを書き込む`(controller → service → Redis、 TTL が 0 < ttl ≤ 900 sec を assert)、 (2) `revocation_キー有り_の_userId_の_request_は_RevocationCheckFilter_で_401`(Redis 直書き → filter 401 検証、 SUPER_ADMIN role 持ちでも sub=revoked なら filter が先に弾く)、 (3) `revocation_無し_user_は_request_が_controller_に到達_して_200`(filter 素通しの baseline)、 (4) `revoke_tokens_API_経由で_revoke_された_user_の_次_request_は_401`(API + filter 連動の end-to-end)
    - **Spring Boot 起動 trim**: `platform.outbox.publisher-enabled=false`(Kafka 不要、 outbox 書込は audit aspect が使うので有効維持)、 `spring.security.oauth2.resourceserver.jwt.issuer-uri=`(外部 OIDC discovery 抑止、 SecurityConfig が in-process JwtDecoder を提供するので置換不要)
    - **TRUNCATE 範囲**: `users` / `tenant_memberships` / `outbox` を `RESTART IDENTITY CASCADE` で初期化。 audit aspect が `@Auditable` controller 経由で outbox 書込を行うため、 outbox を毎テスト clean しないと別テストの audit log と混在する
    - **Windows ローカル実行**: 既存の Docker Desktop hardening 問題 (CLAUDE.md 記述) で auto-skip。 CI(ubuntu-latest + 素の Docker socket)で初実行
...
- **A5 follow-up¹⁸ tenant deactivate fanout で対象 tenant 内の全 user を一括 revoke**(¹⁷ で「次 phase 候補」 と書いた最後のピース。 ¹⁵ の user 単体 deactivate / ¹⁴ の membership 取消は per-user revoke で済むが、 tenant 全体 deactivate の場合は対象 tenant に紐付く全 membership user を一括無効化する必要がある。 これで tenant offboarding の意味的な完成度が user offboarding と対称になる):
    ...
    - **port-out impl**: `TenantMembershipRepositoryImpl.findUserIdsByTenant` で `Long → UserId` 変換
    - **`TenantManagementService` に 2 新引数注入**(`TenantMembershipRepository` / `RevocationStore`) + `REVOCATION_TTL = Duration.ofMinutes(15)` 定数。 `deactivate` で `repository.update(tenant)` 後に `findUserIdsByTenant(id)` → ループで `revocationStore.revokeUser(userId.value(), TTL)`。 同 TX 内呼出だが Redis は非トランザクションのため fail-open(`RedisRevocationStore` 側で `DataAccessException` を warn 抑制)。 INFO ログに `revoke 登録ユーザ数={size}` を出す
    - **`per-user 単位 revoke` の意味的影響**: 該当 user に他テナントへの membership が残っていれば、 再 login で残テナントだけ scope に含む新 token を取れる(`SelectTenantService` が DEACTIVATED tenant を弾く既存挙動と組合わさって正しく動く)
    - **テスト 1 ケース追加 + 既存 4 ケースに `revoke` assertion**(10/10 全通過): `deactivate_は_membership_0件_でも正常終了し_revoke_を呼ばない` / `ACTIVE → DEACTIVATED` で 2 user 分 verify / 冪等呼出でも revoke 再登録 / `TenantNotFoundException` / `TenantProtectedException` で `revokeUser` を `never()` 確認
    - **これで identity-broker 全 user/tenant lifecycle で ADR-0023 即時 revocation が完全に効く**: `user.deactivate` / `removeMembership` (¹⁷) + `tenant.deactivate` (¹⁸)。 残りの「想定外 token 漏出時の admin 強制 revoke API」 は別 follow-up(MVP では runbook で `redis-cli SET revocation:user:<id> X EX 900` で対応)
- **A5 follow-up¹⁷-hotfix Redis 不在 service の Spring context 起動失敗を修正**(¹⁷ 初版で `PlatformSecurityAutoConfiguration` の `redisRevocationStore` method 引数に `StringRedisTemplate` 型を直接書いていたため、 inventory-core 等 spring-data-redis 非依存 service が CI 上 `Failed to introspect Class[...PlatformSecurityAutoConfiguration]` で context 初期化に失敗していた。 Java reflection の `getDeclaredMethods()` が parameter type を解決する際に `NoClassDefFoundError: StringRedisTemplate` を発する):
    - **fix**: Redis 実装を nested `@Configuration(proxyBeanMethods=false) RedisRevocationStoreConfiguration` に分離 + class-level `@ConditionalOnClass(name=...)`(string lookup)で囲む。 nested class は条件不成立時に class-load 自体が起きないため、 reflection も走らず `NoClassDefFoundError` を回避できる
    - **regression test 追加**: `PlatformSecurityAutoConfigurationTest` 2 ケース。 (1) `FilteredClassLoader` で Redis 関連 class を classpath から隠した状態で `ApplicationContextRunner` を起動 → `NoOpRevocationStore` が単一 `RevocationStore` Bean として fallback、 (2) Redis classpath 有り + `RedisConnectionFactory` Bean 無しでも `NoOpRevocationStore` を採る。 commons-security 5/5 全通過
- **A5 follow-up¹⁷ ADR-0023 即時 token revocation を実装(commons-security 共通フィルタ + identity-broker 発行側)**(¹⁶ で accepted した設計を 12 services 全部に効かせる実装。 user offboarding (deactivate / removeMembership) 直後に access token を即時無効化、 残 TTL アクセスを封じる):
    - **`commons-security`**: `RevocationStore` port 抽象 + 2 実装(`NoOpRevocationStore` = default、 `RedisRevocationStore` = `spring-boot-starter-data-redis` 検出時に `@ConditionalOnClass(StringRedisTemplate)` + `@ConditionalOnBean(RedisConnectionFactory)` で有効化)。 `RedisRevocationStore` は `revocation:user:<userId>` キーで TTL=15min 自己消滅、 `DataAccessException` は warn log のみで fail-open
    - **`RevocationCheckFilter`** (`OncePerRequestFilter`、 `BearerTokenAuthenticationFilter` 後 / `TenantContextFilter` 前) を新設。 認証済 JWT principal の `sub` を Long に解釈 → `store.isUserRevoked()` → revoked なら `SecurityContextHolder.clearContext()` + `entryPoint.commence(DisabledException)` で 401 RFC 7807 を返す。 未認証 (無 JWT) は素通し
    - **`PlatformSecurity`** に `RevocationCheckFilter` を 5th 引数として注入、 filter chain を `Bearer → Revocation → TenantContext` の順に組み立て直し(従来の `Bearer → TenantContext` 直結を間に挟み込み)
    - **`PlatformSecurityAutoConfiguration`** に 3 bean (`redisRevocationStore` / `noOpRevocationStore` / `revocationCheckFilter`) を `@ConditionalOn` 付きで追加。 Redis を入れていない 13 services のうち identity-broker 以外は `NoOpRevocationStore` で no-op、 zero-cost
    - **`commons-security` pom**: `spring-boot-starter-data-redis` を `optional=true` で追加(消費 service が opt-in)。 `spring-boot-starter-test` を test scope で追加(JUnit 5 + Mockito 確保)
    - **identity-broker pom**: `spring-boot-starter-data-redis` を mandatory 依存として追加(本 service が publisher 側で必須)。 `application-test.yml` に Redis 設定不要(NoOpRevocationStore がテストで起動)
    - **`UserManagementService`** に 6th コンストラクタ引数 `RevocationStore` 注入 + `REVOCATION_TTL = Duration.ofMinutes(15)` 定数。 `deactivate(userId)` と `removeMembership(userId, tenantId)` の DB 操作成功直後に `revocationStore.revokeUser(userId.value(), REVOCATION_TTL)` を呼び、 `Auditable` log に `revoke 登録済(ADR-0023)` を残す
    - **テスト追加**: `commons-security/RevocationCheckFilterTest` 3 ケース(未認証は素通し / revoked なら chain 呼ばず 401 / revoke 無しは素通し)、 `UserManagementServiceTest` の `deactivate` / `removeMembership` 既存ケースに `verify(revocationStore).revokeUser(...)` を追加(commons-security 3/3 + identity-broker 59/59 全通過)
    - **次 phase 候補**: tenant deactivate fanout(`TenantManagementService.deactivate` で対象 tenant の全 membership user を revoke、 `TenantMembershipRepository.findUserIdsByTenant(TenantId)` の port-out 拡張が必要)

...
- **REST API の Pact 契約**: BFF ↔ 各サービスや、サービス間 gRPC/REST がもし生まれたとき(現状は Kafka 中心)に追加。
- **Compatibility matrix の自動化**: `can-i-deploy` で「この PR を merge して Production に出していいか」を判定。
...
- Phase 2.1: `wholesale-service` 側で `inventory-core-wholesale.json` を verify する JUnit テスト
- Phase 2.2: Consumer 契約の追加(retail / manufacturing / shipped / cancelled / 補償経路)
- Phase 2.3: Pact Broker 導入の re-evaluation(契約 5 件超 + Provider verify 安定後)

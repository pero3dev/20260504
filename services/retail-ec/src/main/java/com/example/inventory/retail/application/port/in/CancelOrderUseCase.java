...
/**
 * 注文キャンセル(業務側起因)ユースケース。
 *
 * <p>状態 PLACED → CANCELLED に遷移し、{@code retail.order.cancelled.v1} を Outbox 経由で発行する。 Inventory Core
 * はこれを listener で受けて {@code Inventory.release} を呼び reserved を解放する(ADR-0018)。
 *
 * <p>同一 PLACED 注文への重複 cancel は冪等(状態のみ遷移、 イベント発行なし)。CANCELLED で再呼出は no-op。 SHIPPED からは {@code

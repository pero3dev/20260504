...
/**
 * 注文キャンセル確定イベント。{@code retail.order.cancelled.v1} トピックへ Outbox 経由で発行される。
 *
 * <p>業務側で取消が確定した時点で発行 — Inventory Core はこれを受けて該当の reserved を解放(release)する。 これにより 「注文キャンセル後に
 * reserved が宙ぶらりんで戻らない」整合性破綻を解消する。
 *
 * <p>引当補償({@code retail.reservation.failed.v1})とは独立: 補償は「Reserve に失敗 → Order を取消」、 本イベントは「業務側で取消 →
 * Reserved を解放」と方向が逆。
 */
public record OrderCancelledEvent(
    ...
    public static final String TOPIC = "retail.order.cancelled.v1";

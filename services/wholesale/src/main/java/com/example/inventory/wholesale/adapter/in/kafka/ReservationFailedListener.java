...
/**
 * {@code wholesale.reservation.failed.v1} を購読する Kafka リスナ。
 *
 * <p>Inventory Core からの補償通知を受けて SalesOrder を CANCELLED に遷移させる。 業態ごとに補償トピックを分離しているため、Retail/EC の
 * 補償(別 listener)とは独立。
 */
@Component
public class ReservationFailedListener {
    ...
    @KafkaListener(
            topics = "wholesale.reservation.failed.v1",
    ...
    public void onReservationFailed(ConsumerRecord<String, String> record, Acknowledgment ack)
        ...
        if (tenantIdValue == null) {
            LOG.warn(
                    "tenant_id 無しの wholesale.reservation.failed.v1 をスキップ partition={} offset={}",
                    record.partition(),
                    record.offset());
            ack.acknowledge();
            return;

...
/**
 * {@code retail.order.shipped.v1} を購読する Kafka リスナ。
 *
 * <p>Retail/EC から飛んできた出荷確定イベントを受けて、{@link ShipForOrderUseCase} に渡し各明細を ship する。 Wholesale の {@code
 * SalesOrderShippedListener} と完全並列。groupId のみ業態別に分離。
 *
 * <p>ADR-0017 に従い MVP では失敗時の補償は持たず、業務エラーは @Transactional ロールバック → ack されないため Spring Kafka 既定 retry
 * → DLQ で観察する。
 *
 * <p>Reserve は Phase 1+2 の {@code OrderPlacedListener} で済んでいる前提なので、 reserved 不足になることは
 * 通常は無い(なれば運用上のデータ整合性破綻として人が見る)。
 */
@Component
public class RetailOrderShippedListener {

    private static final Logger LOG = LoggerFactory.getLogger(RetailOrderShippedListener.class);
    ...
    public RetailOrderShippedListener(ShipForOrderUseCase shipUseCase, ObjectMapper objectMapper) {
        this.shipUseCase = shipUseCase;
        this.objectMapper = objectMapper;
    }

    @KafkaListener(
            topics = "retail.order.shipped.v1",
            groupId = "${spring.application.name}-retail-order-ship")
    public void onRetailOrderShipped(ConsumerRecord<String, String> record, Acknowledgment ack)
            throws Exception {
        String tenantIdValue = headerValue(record, HEADER_TENANT_ID);
        if (tenantIdValue == null) {
            LOG.warn(
                    "tenant_id 無しの retail.order.shipped.v1 をスキップ partition={} offset={}",
                    record.partition(),
                    record.offset());
            ack.acknowledge();
            return;
        ...
        RetailOrderShippedMessage msg =
                objectMapper.readValue(record.value(), RetailOrderShippedMessage.class);
        ...
        try {
            ...
            ack.acknowledge();
            LOG.debug(
                    "Retail Order {} の出荷確定を反映({}行) tenant={}",
                    msg.code(),
                    msg.items().size(),
                    tenantIdValue);
        } finally {

...
**federation 経路の SUPER_ADMIN provisioning** は `FederationJitProperties.groupRoleMappings` で対応(A5 follow-up¹¹)。 IdP の groups claim (Cognito の `cognito:groups`、 Azure AD の `groups` 等) と内部ロールを map で紐付ける:

```yaml
platform:
  identity:
    federation:
      groups-claim: "cognito:groups"   # IdP 側 claim 名(Cognito 既定)
      jit:
        enabled: true
        default-tenant-id: platform
        default-role: VIEWER            # 後ろ向き互換 / mapping 不一致時の fallback
        group-role-mappings:
          platform-admins: SUPER_ADMIN
          inventory-managers: INVENTORY_MANAGER
```

最初に合致した group の role が採用される(map iteration 順)。 上記設定 + Cognito 側で `platform-admins` group に admin user を入れれば、 federated 認証で初回ログイン時に platform tenant + SUPER_ADMIN role の membership が JIT で作られる。 ops が SQL を叩く必要が無い。
...
- user 管理 admin API:read 系 `GET /v1/admin/users`, `GET /v1/admin/users/{userId}` 実装済(`@Auditable read=true`)。 write 系は federation-only register `POST /v1/admin/users` (A5 follow-up¹²)、 cross-tenant 追加 membership `POST /v1/admin/users/{userId}/memberships` (¹³)、 個別 offboarding `DELETE /v1/admin/users/{userId}/memberships/{tenantId}` (¹⁴)、 user 全体 deactivate `POST /v1/admin/users/{userId}/deactivate` (¹⁵、 UserStatus 列追加 + AuthenticateService / ExchangeFederatedTokenService 双方で DEACTIVATED user を 401 で弾く) で完了。 即時 token revocation (stateful list) は別 phase

...
- user 管理 admin API:read 系 `GET /v1/admin/users`, `GET /v1/admin/users/{userId}` 実装済(`@Auditable read=true`)。 write 系は federation-only register が `POST /v1/admin/users` (A5 follow-up¹²)、 cross-tenant 追加 membership が `POST /v1/admin/users/{userId}/memberships` (¹³)、 offboarding が `DELETE /v1/admin/users/{userId}/memberships/{tenantId}` (¹⁴) で実装済。 user 全体の deactivate (UserStatus 列追加 + 全 membership 一括無効化) は次 phase

...
- user 管理 admin API:read 系 `GET /v1/admin/users`, `GET /v1/admin/users/{userId}` 実装済(`@Auditable read=true`)。 write 系は federation-only register が `POST /v1/admin/users` で実装済(A5 follow-up¹²)。 既存 user への membership 追加は `POST /v1/admin/users/{userId}/memberships` で実装済(A5 follow-up¹³、 cross-tenant ユーザ provisioning 対応)。 deactivate / unlink-membership は次 phase

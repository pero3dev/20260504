## Cognito User Pool + SAML Federation — 本番デプロイランブック(F2 phase C)
...
infra/cognito/
├── README.md                 # 本ファイル
├── userpool.tf               # ※ 後フェーズ: Terraform / CDK で IaC 化
└── attribute-mapping.json    # SAML assertion attribute → Cognito user attribute
```

MVP は本 README の手順に従って AWS Console / CLI で構築する。 IaC 化は後フェーズ(本タスクでは対象外)。
...
- CLI 例は MVP 用。 production は Terraform / CDK で IaC 化する(後タスク)
...
- JIT provisioning は本 phase 未実装(Identity Broker DB に予め user を投入しておく)。 SCIM 連携 / 後続バッチで自動化する別タスクを検討

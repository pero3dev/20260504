...
- **Audit collection is AOP-only (`@Auditable`), no CDC.** Mitigations: ArchUnit `HexagonalLayerRules.writePathsAreAuditable` enforces `@Auditable` (or `@AuditExempt` with `reason`) on every `..application.usecase..` class that calls a `*Repository` write — fully opt-in across all 13 services as of A5 follow-up¹⁰; Aurora WAL stored separately; DBA actions go through a separate channel. Don't drop these mitigations.
